...
navbar_pattern = r'<div data-animation="default" class="navbar [^"]*w-nav"[^>]*>.*?</div></div></div></div></div>'
replacement = '''<header class="top-header">
  <style>
    .hero-top-nav {
      position: absolute;
      top: 0;
...
new_content = re.sub(navbar_pattern, replacement, content, flags=re.DOTALL)
